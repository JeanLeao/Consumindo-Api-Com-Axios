...
function createGame(){ // FUNÇÃO DE CRIAR O GAME
    ...
    axios.post('http://localhost:1234/game',game).then((r)=>{
        if(r.statusCode == 200){
...
function deleteGame(list){  // FUNÇÃO DE DELETE NO GAME
    ...
    console.log(name)
    axios.delete('http://localhost:1234/game/'+`${name}`).then(() => {
        alert('game deletado');
    }).catch((err) => {
        console.log(err)
    })
...
function UpdateClick(){  // FAZENDO O UPDATE PELO CLICK DO BOTÃO
    ...
    axios.put('http://localhost:1234/game/'+IdField.value,game).then((r)=>{
        if(r.statusCode == 200){
            alert('Game Editado!');
        }
    }).catch((err)=>{
...
axios.get('http://localhost:1234/games').then((res) => { // RETORNANDO OS DADOS DO GAME
